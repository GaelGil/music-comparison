...
class GetSpotifyPlaylistData:
    ...
    def get_spotify_playlists(self, genres:list, limit:int, type=None):
        ...
        if type == 'search':
            ...
            playlist_data = [self.spotify_client.search(q=i, limit=limit, offset=j, type='playlist', market='US') for i in genres for j in range(10)]
        if type == 'featured':
            # if we only want featured playlists
            playlist_data = [self.spotify_client.category_playlists(category_id=i, country='US', limit=limit, offset=j) for i in genres for j in range(3)]
        if not type:
            # if we want both
